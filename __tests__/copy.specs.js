...
const SOURCE_KEY = 'redis-ts:copy:src';
const DEST_KEY = 'redis-ts:copy:dest';
...
describe('copy', () => {
  ...
  describe('options', () => {
    ...
    test('limit', async () => {
      ...
      await copy(client, SOURCE_KEY, DEST_KEY, start_ts, start_ts + samples_count, 'LIMIT', 4);
      const response = await getRange(client, DEST_KEY, start_ts, start_ts + samples_count);
      const actual = response.map(x => parseInt(x[1].value));
      expect(actual.length).toEqual(4);
      expect(actual[0]).toEqual(data[0]);
      expect(actual[3]).toEqual(data[3]);
    });
    ...
    test('aggregation', async () => {
      ...
      const actual = response.map(x => {
        const { id } = parseId(x[0]);
        return [id, parseInt(x[1].value_count)];
      });
      expect(actual).toEqual(expected);
    });
    ...
    describe('storage', () => {
      ...
      describe('hash', () => {
        ...
        test('non-aggregation', async () => {
          ...
          const dest = `${DEST_KEY}:hash`;
          await insertData(client, SOURCE_KEY, start_ts, data.length, data);
          await copy(client, SOURCE_KEY, dest, start_ts, start_ts + data.length, 'STORAGE', 'hash' );

          const response = await getHash(dest);
